...
contract EARNPRIVATE is Ownable, ReentrancyGuard {
    ...
    // Evento personalizado para emitir información sobre el depósito del usuario
    event DepositUpdated(address indexed user, uint256 depositId, uint256 amount, uint256 initTime, uint256 finishTime, uint256 earn);
    ...
    // Modificador para verificar si el contrato está inicializado
    modifier onlyInitialized() {
        require(_initialized, "Pool is not initialized");
        _;
    }

    // Modificador para verificar si el usuario está en la lista blanca
    ...
    function Initialized(uint256 _limit,uint256 _maxAmount, uint256 _minAmount, uint256 _percent,uint256 _days) public onlyOwner{
        ...
        // Initialize the variables that are not initialized in the constructor
        recolect = 0;
        _lock = false;
        limit = _limit;
    ...
    function Deposit(uint256 _amount) external nonReentrant ContractLock {
        ...
        // Utiliza la dirección del token especificada en el constructor
        address tokenContract = TokenRecibe;

        // Crea una instancia del contrato del token
        IERC20 token = IERC20(tokenContract);

        // Llama a la función approve para permitir que el contrato EARNPRIVATE gaste tokens en tu nombre
        bool approveSuccess = token.approve(address(this), _amount);
        require(approveSuccess, "Approve failed");

        // Verifica el monto del depósito
        require(depositCount[msg.sender] < limit, "User has reached deposit limit");
        uint256 balance = token.balanceOf(msg.sender);
        require(balance >= _amount, "Insufficient balance");
        require(_amount > 0, "Amount must be greater than zero");
        require(_amount >= MinAmount, "Amount is less than the minimum allowed");
        require(_amount <= MaxAmount, "Amount is greater than the maximum allowed");
        require(recolect + _amount <= limit, "Contract limit exceeded");

        // Effects
        Users memory userInfo = Users(0, 0, 0, 0);
        userInfo.Amount = _amount;
        userInfo.initTime = block.timestamp;
        userInfo.finishTime = block.timestamp + TimeDays;
        userInfo.Earn = _amount.mul(PercentEarn).div(100);
        balanceUser[msg.sender].push(userInfo);
        depositCount[msg.sender]++;
        recolect += _amount;

        // Interactions
        uint256 depositId = balanceUser[msg.sender].length;
        bool transferSuccess = token.transferFrom(msg.sender, walletPool, _amount);
        require(transferSuccess, "Transfer failed");
        emit DepositMade(msg.sender, _amount, depositId);
    }

    function EarnDeposit(uint256 _depositId) external nonReentrant ContractLock onlyInitialized onlyWhitelisted {
        // Verifica el depósito del usuario
        Users[] storage userDeposits = balanceUser[msg.sender];
        require(_depositId < userDeposits.length, "Invalid deposit ID");
        Users storage userInfo = userDeposits[_depositId];
        ...
        // Verifica si el usuario ya ha retirado sus recompensas
        require(userInfo.Earn > 0, "You have already withdrawn your rewards");

        // Effects
        uint256 withdrawalAmount = userInfo.Earn;
        userInfo.Amount = 0;
        userInfo.Earn = 0;

        // Interactions
        emit WithdrawalMade(msg.sender, withdrawalAmount, _depositId);
        emit DepositUpdated(msg.sender, _depositId, userInfo.Amount, userInfo.initTime, userInfo.finishTime, userInfo.Earn);
        bool success = IERC20(TokenRecibe).transfer(msg.sender, withdrawalAmount);
